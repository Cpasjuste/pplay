...
static void video_refresh(void *opaque, double *remaining_time) {
    ...
    printf("check_external_clock_speed\n");
    if (!is->paused && get_master_sync_type(is) == AV_SYNC_EXTERNAL_CLOCK && is->realtime)
        check_external_clock_speed(is);
    printf("check_external_clock_speed\n");

    if (!display_disable && is->show_mode != SHOW_MODE_VIDEO && is->audio_st) {
        printf("av_gettime_relative\n");
        time = av_gettime_relative() / 1000000.0;
        if (is->force_refresh || is->last_vis_time + rdftspeed < time) {
            printf("video_display (is->force_refresh || is->last_vis_time)\n");
            video_display(is);
    ...
    printf("is->video_st 1\n");
    if (is->video_st) {
        printf("is->video_st 2\n");
        retry:
        printf("frame_queue_nb_remaining\n");
...
int main(int _argc, char **_argv) {
    ...
    char *argv[] = {
            ...
            "debug",
            "-i",
            "Serenity - HD DVD Trailer.mp4",
            NULL
    };
    int argc = 7;

    //input_filename = "FPS_test_1080p23.976_L4.1.mkv";
